...
initialX = table(20,50,1.4583,'VariableNames',{'Ep','Ef','Gfp'});
fun = @(x)myscript(x);
results = bayesopt(fun,vars,'Verbose',0,...
    'AcquisitionFunctionName','expected-improvement-per-second-plus', ...
    'ExplorationRatio',.65,'IsObjectiveDeterministic',true, ...
    'MaxObjectiveEvaluations',150,'ParallelMethod','clipped-model-prediction',...
    'NumSeedPoints',50,'InitialX',initialX); % MinWorkerUtilization=3
